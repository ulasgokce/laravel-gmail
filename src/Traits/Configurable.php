...
trait Configurable
{
    ...
    public function config($user_id = null, $email = null, $string = null)
    {
        $integraion = GmailSenderIntegration::where('email', $email)->where('user_id', $user_id)->first();
        if (isset($integraion)) {
            $config = json_decode($integraion->config, true);
            if ($string) {
